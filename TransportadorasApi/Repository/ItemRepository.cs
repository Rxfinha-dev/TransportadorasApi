...
namespace TransportadorasApi.Repository
{
    public class ItemRepository : IItemRepository
    {
        ...
        {
            return _context.Itens.Where(i=>i.Id ==  id).FirstOrDefault();
        }

        public ICollection<Item> GetItens()
        {
            return _context.Itens.OrderBy(i => i.Id).ToList();
        }

        public ICollection<Pedido> GetPedidosByItem(int itemId)
        {
            return _context.PedidoItems.Where(i => i.ItemId == itemId).Select(p=>p.Pedido).ToList();            
        }
